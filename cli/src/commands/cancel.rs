...
pub fn cancel(
    ...
    // Fetch wallet to get name and intent count
    ...
    // Fetch proposal for params_data
    ...
    if status != STATUS_ACTIVE && status != STATUS_APPROVED {
        anyhow::bail!("Proposal cannot be cancelled (status: {})", status_to_str(status));
    }
    ...
    // Cancel instruction: disc=12
    let ix_data = vec![12u8];
    ...
    let cancel_ix = Instruction::new_with_bytes(program_id, &ix_data, accounts);

    let recent_blockhash = client.get_latest_blockhash()?;
    let tx = Transaction::new_signed_with_payer(
        &[ed25519_ix, cancel_ix],
        Some(&payer.pubkey()),
        &[&payer],
        recent_blockhash,
    );
    ...
    println!("{}", "Proposal cancelled!".yellow().bold());
