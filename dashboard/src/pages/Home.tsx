...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center pt-16 pb-12">
      {/* Hero */}
      <div className="w-16 h-16 rounded-2xl bg-amber-500/15 border border-amber-500/25 flex items-center justify-center mb-6 shadow-glow-gold">
      ...
      <h1 className="text-4xl font-bold text-amber-50 mb-2 font-heading tracking-wider text-glow-gold">Lucid</h1>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg mb-16">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 transition-all text-base"
            autoFocus
          />
          <button
            type="submit"
            className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-lg transition-colors cursor-pointer shadow-glow-purple"
          >
            View
          </button>
        </div>
